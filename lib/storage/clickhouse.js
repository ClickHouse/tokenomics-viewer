...
const { canonicalJson, sha256 } = require("../core/usage-event-identity");
...
function createClickHouseBackend(dependencies = {}) {
  ...
  async function clickHouseGeneration(client, generationId) {
  ...
  async function loadClickHouseCodexSessionHeaders(client, generationId) {
    ...
    return clickHouseJsonEachRow(client, `
      ...
      SELECT
        ...
        headers.source_path AS source_path,
      ...
      GROUP BY headers.session_id, headers.source_path
      ORDER BY headers.session_id, headers.source_path
    `, { params: { generation: generationId } });
  }
  ...
  async function storeClickHouseCodexSessionHeaders(client, headers, sourceStates, changedSources, options = {}) {
    ...
    for (const header of headers || []) {
      ...
      rowsBySession.set(`${sessionId}\u0000${source.sourcePath}`, {
        sessionId,
  ...
  function removeSupersededClickHouseSources(
    sourceStates,
    changedSources,
    manifestChanges,
    currentHeaders,
    ...
    const persistedSourcesBySession = new Map();
    for (const header of persistedHeaders || []) {
      ...
      if (!sessionId || !sourcePath) continue;
      if (!persistedSourcesBySession.has(sessionId)) persistedSourcesBySession.set(sessionId, new Set());
      persistedSourcesBySession.get(sessionId).add(sourcePath);
    }
    ...
    for (const header of currentHeaders || []) {
      ...
      const currentState = sourceStates.get(sourcePath);
      for (const previousPath of persistedSourcesBySession.get(sessionId) || []) {
        if (previousPath === sourcePath) continue;
        const decisionKey = `${sessionId}\u0000${previousPath}\u0000${sourcePath}`;
        if (decisions.has(decisionKey)) continue;
        decisions.add(decisionKey);
        const previousState = sourceStates.get(previousPath);
        const fingerprintMatch = sameSourceContentFingerprint(
          previousState?.fingerprint,
          currentState?.fingerprint,
        );
        const action = fingerprintMatch ? "removed_matching_source" : "retained_continuation";
        logProgress(options, syncMetricLine("same_session_source", {
          runId: options.syncDiagnosticRunId,
          engine: "clickhouse",
          sessionId: diagnosticId(sessionId),
          previousSourceId: diagnosticId(previousPath),
          currentSourceId: diagnosticId(sourcePath),
          fingerprintMatch,
          action,
        }));
        if (fingerprintMatch && sourceStates.delete(previousPath)) {
  ...
  function logClickHouseSourceImport(options, source, staged, mode, reason, bytes) {
    logProgress(options, syncMetricLine("source_import", {
      runId: options.syncDiagnosticRunId,
      engine: "clickhouse",
      sourceId: diagnosticId(source.path),
      kind: source.kind,
      adapter: source.inputMeta?.adapter || null,
      mode,
      reason,
      bytes: Number(bytes ?? source.sizeBytes) || 0,
      segmentStart: Number(staged.sourceState.segment_start) || 0,
      segmentEnd: Number(staged.sourceState.segment_end) || 0,
      ...sourceReportMetric(staged.report),
    }));
  }
  ...
  async function syncClickHouseJsonlSource(client, input, sourceStates, options) {
    ...
    if (!appendable) {
      ...
      logClickHouseSourceImport(options, {
        kind: "jsonl",
        path: input.path,
        sizeBytes: stat.size,
        inputMeta: input,
      }, staged, "full", "fingerprint_changed", stat.size);
    ...
    logClickHouseSourceImport(options, {
      ...
      sizeBytes: stat.size,
      inputMeta: input,
    }, staged, plan.mode, plan.reason, plan.end - plan.start);
    return nextState;
  }
  ...
  async function syncClickHouseZipSource(client, input, sourceStates, changedSources, manifestChanges, options, limiter) {
    ...
    for (const entry of entries) {
      ...
      logClickHouseSourceImport(options, source, staged, "full", "fingerprint_changed", entry.uncompressedSize);
  ...
  function clickHouseGlobalUsageKey(alias = "raw") {
    return `if(
      startsWith(${alias}.event_key, 'request:'),
      concat('request:', ${alias}.provider, ':', substring(${alias}.event_key, 9)),
      concat('source:', ${alias}.source_path, ':', if(${alias}.event_key = '', concat('line:', toString(${alias}.line_no)), ${alias}.event_key))
    )`;
  }

  function clickHouseCommittedRowsDefinition(table, name, { globalUsageDedupe = true } = {}) {
    const activeName = `${name}_active`;
    const sourceName = table === "usage_events" && globalUsageDedupe ? `${name}_source` : name;
    ...
    return `
      ...
      ${table === "usage_events" && globalUsageDedupe ? `,
      ${name}_keyed AS (
        SELECT *, ${clickHouseGlobalUsageKey(sourceName)} AS _logical_event_key
        FROM ${sourceName}
      ),
      ${name} AS (
        SELECT *
        FROM ${name}_keyed
        ORDER BY timestamp ASC, source_path ASC, import_id ASC, line_no ASC
        LIMIT 1 BY _logical_event_key
      )` : ""}
    `;
  }

  async function applyClickHouseUsageIdentityDiagnostics(client, report, generationId) {
    const rows = await clickHouseJsonEachRow(client, `
      WITH ${clickHouseActiveManifestDefinitions()},
      ${clickHouseCommittedRowsDefinition("usage_events", "identity_usage_events", { globalUsageDedupe: false })},
      duplicate_keys AS (
        SELECT
          ${clickHouseGlobalUsageKey("raw")} AS logical_event_key,
          count() AS copies
        FROM identity_usage_events AS raw
        GROUP BY logical_event_key
        HAVING copies > 1
      ),
      conflict_keys AS (
        SELECT ${clickHouseGlobalUsageKey("raw")} AS logical_event_key
        FROM identity_usage_events_active AS raw
        GROUP BY logical_event_key
        HAVING uniqExact(tuple(
          provider, model, project, effort, service_tier, service_mode, agent,
          input, cache_create_5m, cache_create_30m, cache_create_1h, cache_read,
          output, reasoning_output, context_window,
          visible_input_chars, visible_output_chars, visible_total_chars
        )) > 1
      )
      SELECT
        sum(copies - 1) AS duplicateUsageEvents,
        (SELECT count() FROM conflict_keys) AS conflictingUsageEvents
      FROM duplicate_keys
    `, { params: { generation: generationId } });
    const diagnostics = rows[0] || {};
    report.sources.duplicateUsageEvents = number(diagnostics.duplicateUsageEvents);
    report.sources.conflictingUsageEvents = number(diagnostics.conflictingUsageEvents);
    if (report.sources.conflictingUsageEvents > 0) {
      throw new Error(`Conflicting stored usage events: ${report.sources.conflictingUsageEvents}`);
    }
  }

  async function clickHouseUsageProvenance(client, generationId) {
    const rows = await clickHouseJsonEachRow(client, `
      ${clickHouseGenerationCte("usage_events", "committed_usage_events")}
      SELECT
        lower(hex(SHA256(concat(
          toString(count()), ':',
          toString(sum(cityHash64(payload))), ':',
          toString(groupBitXor(cityHash64(payload))), ':',
          toString(min(cityHash64(payload))), ':',
          toString(max(cityHash64(payload)))
        )))) AS eventSetDigest,
        formatDateTime(max(parsed_timestamp), '%Y-%m-%dT%H:%i:%S.%fZ', 'UTC') AS dataThrough
      FROM (
        SELECT
          concat(
            _logical_event_key,
            '=',
            toJSONString(tuple(
              timestamp, provider, model, project, effort, service_tier, service_mode, agent,
              input, cache_create_5m, cache_create_30m, cache_create_1h, cache_read,
              output, reasoning_output, context_window,
              visible_input_chars, visible_output_chars, visible_total_chars
            ))
          ) AS payload,
          parseDateTime64BestEffortOrNull(toString(timestamp)) AS parsed_timestamp
        FROM committed_usage_events
        ORDER BY _logical_event_key
      )
    `, { params: { generation: generationId } });
    const row = rows[0] || {};
    return {
      eventSetDigest: row.eventSetDigest || null,
  ...
  async function syncClickHouseDatabase(options) {
    ...
    const syncDiagnosticRunId = randomUUID();
    ...
    const syncOptions = { ...configuredOptions, syncDiagnosticRunId };
    ...
    const inputs = await discoverInputs(syncOptions);
    ...
    logProgress(syncOptions, syncMetricLine("discovery", {
      runId: syncDiagnosticRunId,
      engine: "clickhouse",
      totalSources,
    ...
    logProgress(syncOptions, `[clickhouse] changed source candidates=${formatInt(changedSourcePaths.size)}`);
    ...
    logProgress(syncOptions, syncMetricLine("report_snapshot", {
      runId: syncDiagnosticRunId,
      engine: "clickhouse",
      changedSources: changed,
      generationId: diagnosticId(committed?.generation_id || ""),
      ...reportMetric(report),
    }));
    logProgress(syncOptions, `[clickhouse] ${clickHouseLabel(client)} changed_sources=${formatInt(changed)} sessions=${formatInt(report.sessions.length)}`);
    return report;
  }
  ...
  async function buildReportFromClickHouse(options = {}, pinnedGenerationId = null) {
    ...
    const generationId = pinnedGenerationId
      ...
      || "";
    const report = newReport();
    await applyClickHouseUsageIdentityDiagnostics(client, report, generationId);
    ...
    const [generation, sourceStates, usageProvenance] = await Promise.all([
      clickHouseGeneration(client, generationId),
      loadClickHouseGenerationSources(client, generationId),
      clickHouseUsageProvenance(client, generationId),
    ]);
    report.provenance = {
      generationId,
      committedAt: generation ? new Date(number(generation.committed_at_ms)).toISOString() : null,
      sourceManifestDigest: sha256(canonicalJson([...sourceStates.entries()].sort(([left], [right]) => left.localeCompare(right)))),
      codexUsageDerivationComplete: [...sourceStates.values()].every(({ fingerprint }) => (
        codexUsageDerivationVersionFromFingerprint(fingerprint) === CODEX_USAGE_DERIVATION_VERSION
      )),
      ...usageProvenance,
    };
    return report;
