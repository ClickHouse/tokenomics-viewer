...
function createClickHouseBackend(dependencies = {}) {
  ...
  async function insertClickHousePricingOverlays(client, configuration, generationId) {
    ...
    `, { params: { generation: generationId, pricingRevision: configuration.revision } });
  ...
  async function applyClickHouseRateLimits(client, report, generationId, configuration) {
    ...
    const bucketRows = await clickHouseJsonEachRow(client, `
      ...
      SELECT
        ...
        argMax(used_percent, tuple(timestamp_ms, sample_sequence, sample_source_path, sample_line_no)) AS latestUsedPercent,
