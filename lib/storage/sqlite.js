...
const {
  ...
  usageEventKey,
  usagePayloadHash,
} = require("../core/usage-event-identity");
...
function createSqliteBackend(dependencies = {}) {
  ...
  function sqliteMeta(db, key) {
    return db.prepare("SELECT value FROM meta WHERE key = ?").get(key)?.value || null;
  }

  function commitSqliteReportGeneration(db) {
    const generationId = randomUUID();
    const committedAt = new Date().toISOString();
    const upsert = db.prepare("INSERT OR REPLACE INTO meta(key, value) VALUES(?, ?)");
    db.exec("BEGIN IMMEDIATE");
    try {
      upsert.run("report_generation_id", generationId);
      upsert.run("report_committed_at", committedAt);
      db.exec("COMMIT");
    } catch (error) {
      db.exec("ROLLBACK");
      throw error;
    }
    return { generationId, committedAt };
  }

  function ensureSqliteReportGeneration(db) {
    const generationId = sqliteMeta(db, "report_generation_id");
    const committedAt = sqliteMeta(db, "report_committed_at");
    return generationId && committedAt
      ? { generationId, committedAt }
      : commitSqliteReportGeneration(db);
  }
  ...
  function openTokenomicsDatabase(dbPath) {
    ...
    db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_codex_session_sources_parent ON codex_session_sources(parent_session_id);
      CREATE INDEX IF NOT EXISTS idx_codex_session_sources_path ON codex_session_sources(source_path);
    `);
    db.exec(`
      INSERT OR IGNORE INTO codex_session_sources(
        session_id, parent_session_id, source_path, kind, archive_path, entry_name, updated_at
      )
      SELECT session_id, parent_session_id, source_path, kind, archive_path, entry_name, updated_at
      FROM codex_sessions
    `);
    ...
    ensureSqliteColumn(db, "usage_events", "event_key", "TEXT NOT NULL DEFAULT ''");
    ensureSqliteColumn(db, "usage_events", "payload_hash", "TEXT NOT NULL DEFAULT ''");
    db.exec("CREATE INDEX IF NOT EXISTS idx_usage_events_event_key ON usage_events(event_key)");
    db.prepare("INSERT OR REPLACE INTO meta(key, value) VALUES('schema_version', '2')").run();
    ensureSqliteConfiguration(db);
    ensureSqliteReportGeneration(db);
    return db;
  }
  ...
  function storeSqliteCodexSessionHeaders(db, headers) {
    ...
    const deleteLegacyBySource = db.prepare("DELETE FROM codex_sessions WHERE source_path = ?");
    const deleteSegmentBySource = db.prepare("DELETE FROM codex_session_sources WHERE source_path = ?");
    ...
    for (const row of rows) {
      deleteLegacyBySource.run(row.sourcePath);
      deleteSegmentBySource.run(row.sourcePath);
      const values = [
        row.sessionId,
        row.parentSessionId,
        row.sourcePath,
      ...
      insertLegacy.run(...values);
      insertSegment.run(...values);
    }
  }
  ...
  function deleteSourceRows(db, sourcePath) {
    ...
    db.prepare("DELETE FROM codex_session_sources WHERE source_path = ?").run(sourcePath);
    db.prepare("DELETE FROM sources WHERE source_path = ?").run(sourcePath);
  }

  function deleteSupersededCodexSources(db, sourcePath, fingerprint, sourceHeaders, options) {
    const findSources = db.prepare(`
      SELECT registry.source_path, sources.fingerprint
      FROM codex_session_sources AS registry
      LEFT JOIN sources ON sources.source_path = registry.source_path
      WHERE registry.session_id = ?
      ORDER BY registry.source_path
    `);
    const decisions = new Set();
    for (const header of sourceHeaders || []) {
      const sessionId = normalizeCodexUuid(header?.id);
      if (!sessionId) continue;
      for (const previous of findSources.all(sessionId)) {
        const previousPath = previous.source_path;
        if (!previousPath || previousPath === sourcePath) continue;
        const decisionKey = `${sessionId}\u0000${previousPath}`;
        if (decisions.has(decisionKey)) continue;
        decisions.add(decisionKey);
        const fingerprintMatch = sameSourceContentFingerprint(previous.fingerprint, fingerprint);
        const action = fingerprintMatch ? "removed_matching_source" : "retained_continuation";
        progress(options, syncMetricLine("same_session_source", {
        ...
        if (fingerprintMatch) deleteSourceRows(db, previousPath);
      }
    }
  }
    ...
    return {
      ...
      insertUsage: db.prepare(`
        INSERT INTO usage_events(
          source_path, line_no, event_key, payload_hash, timestamp, date_key, week_key, month_key, year_key,
  ...
  function insertUsageEventRow(statement, event, defaultSourcePath) {
    ...
    const eventKey = event.requestId ? `request:${event.requestId}` : `line:${event.lineNo ?? "unknown"}`;
    statement.run(
      event.sourcePath || defaultSourcePath,
      event.lineNo,
      eventKey,
      usagePayloadHash(event),
      event.timestamp,
      dateKey(timestamp),
      weekKey(timestamp),
      monthKey(timestamp),
  ...
  async function processAndStoreSource(db, source, fingerprint, options, sourceHeaders) {
    ...
    const report = await withAsyncTransaction(db, async () => {
      deleteSupersededCodexSources(db, source.path, fingerprint, sourceHeaders, options);
    ...
    progress(options, syncMetricLine("source_import", {
      runId: options.syncDiagnosticRunId,
      engine: "sqlite",
  ...
  async function syncSqliteDatabase(options) {
    ...
    try {
      ...
      progress(syncOptions, syncMetricLine("discovery", {
      ...
      progress(syncOptions, "[db] changed source candidates=" + formatNumber(changedSourcePaths.size));
      ...
      if (changed > 0) commitSqliteReportGeneration(db);
      const report = buildReportFromOpenDatabase(db, configuredOptions);
      attachSqliteConfigurationState(db, report, configuration);
      progress(syncOptions, syncMetricLine("report_snapshot", {
        runId: syncDiagnosticRunId,
        engine: "sqlite",
        changedSources: changed,
        ...reportMetric(report),
      }));
      progress(syncOptions, "[db] " + dbPath + " changed_sources=" + formatNumber(changed) + " sessions=" + formatNumber(report.sessions.length));
      return report;
    } finally {
      db.close();
    }
  ...
  function buildReportFromOpenDatabase(db, options = {}) {
    ...
    for (const row of db.prepare("SELECT * FROM usage_events ORDER BY timestamp, id").iterate()) {
      const eventKey = usageEventKey(row);
      const payloadHash = row.payload_hash || usagePayloadHash(row);
      const previousHash = usageEvents.get(eventKey);
      if (previousHash) {
        if (previousHash !== payloadHash) {
          report.sources.conflictingUsageEvents += 1;
          throw new Error(`Conflicting stored usage event ${diagnosticEventKey(eventKey)}`);
        }
        report.sources.duplicateUsageEvents += 1;
        continue;
      }
      usageEvents.set(eventKey, payloadHash);
      if (row.timestamp) {
        const timestamp = new Date(row.timestamp);
        if (Number.isFinite(timestamp.getTime())) {
          const candidate = timestamp.toISOString();
          if (!dataThrough || candidate > dataThrough) dataThrough = candidate;
        }
      }
    ...
    const sourceManifest = db.prepare("SELECT source_path, fingerprint FROM sources ORDER BY source_path").all();
    const generation = ensureSqliteReportGeneration(db);
    report.provenance = {
      ...generation,
      dataThrough,
      eventSetDigest: sha256(canonicalJson([...usageEvents.entries()].sort(([left], [right]) => left.localeCompare(right)))),
      sourceManifestDigest: sha256(canonicalJson(sourceManifest)),
      codexUsageDerivationComplete: sourceManifest.every(({ fingerprint }) => (
