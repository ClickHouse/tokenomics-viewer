...
const { attachReportReceipt, closedDayDrift } = require("./core/report-receipt");
...
function createSyncController({
  ...
  function start({ invalidateReport = false } = {}) {
    ...
    pending = Promise.resolve()
      ...
      .then((report) => {
        const finished = now();
        const nextReport = prepareReport(report, {
          committedAt: finished.toISOString(),
          syncRunId: runId,
        });
        const drift = closedDayDrift(reportCache.peek?.(), nextReport, finished);
        reportCache.set(nextReport);
        const progress = status.progress || {};
        ...
        status = {
          ...
          result: {
            ...
            sessions: Array.isArray(nextReport?.sessions) ? nextReport.sessions.length : 0,
            reportReceiptId: nextReport.receipt?.receiptId || null,
            driftClassification: drift.classification,
            closedDayAdditions: drift.additions.length,
          },
          reportReady: true,
        };
        notify();
...
function syncStatusPayload(options) {
  ...
  return {
    ...
    available,
    reportReceiptId: options.reportCache?.peek?.()?.receipt?.receiptId || null,
...
function createWebServer({
      ...
      if (url.pathname === "/api/configuration") {
        ...
        if (request.method === "PUT") {
          ...
          try {
            ...
            const rawReport = canPatchReport
              ? reportWithConfiguration(previousReport, configuration)
              : await buildReportFromSelectedDatabase(options);
            const report = (options.prepareReport || ((value) => value))(rawReport, {
              committedAt: new Date().toISOString(),
              syncRunId: options.syncController?.getStatus().runId ?? null,
            });
  ...
  async function startWebServer(options) {
    ...
    const prepareReport = (report, metadata = {}) => attachReportReceipt(report, {
      runtimeId,
      ...metadata,
    });
    const initialReport = options.preloadedReport
      ? prepareReport(options.preloadedReport, { syncRunId: 0 })
      : null;
    const serverOptions = {
      ...reportOptions,
      runtimeId,
      ...
      reportCache: options.reportCache || createReportCache(
        async () => prepareReport(await buildReportFromSelectedDatabase(reportOptions), { syncRunId: 0 }),
        initialReport,
      ),
    };
