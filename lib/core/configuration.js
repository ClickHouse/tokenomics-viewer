...
const PACKAGED_CONFIGURATION_REVISION = "packaged-7";
...
const PACKAGED_6_CATALOG_SIGNATURE = "f4a4bd86c4ea4a67cdbf0351dc80a3f73e64c24d75ddb9bc9bbe07e534806ebb";
const PACKAGED_6_CLICKHOUSE_CATALOG_SIGNATURE = PACKAGED_6_CATALOG_SIGNATURE;
const PACKAGED_7_CATALOG_SIGNATURE = "ff11f0bfbd3edeb00cf00d995febd78b97abe36a69b7564edf206e8170807605";
...
function rowFromPrices(provider, model, variant, prices, extra = {}) {
  const row = {
    ...
    sourceUrl: provider === "openai" && model === "codex-auto-review" ? ""
             ...
             : provider === "openai" && model === "gpt-6-sol" ? PRICING_SOURCES.openaiGpt6Sol
             : provider === "openai" && model === "gpt-6-luna" ? PRICING_SOURCES.openaiGpt6Luna
             ...
             : model === "claude-opus-5-5" ? PRICING_SOURCES.anthropicOpus55
...
function packagedPricingCatalogRevision(sourceRows) {
  ...
  if (signature === PACKAGED_6_CATALOG_SIGNATURE || signature === PACKAGED_6_CLICKHOUSE_CATALOG_SIGNATURE) {
    return "packaged-6";
  }
  if (signature === PACKAGED_7_CATALOG_SIGNATURE) {
    return PACKAGED_CONFIGURATION_REVISION;
  }
  return "";
}
...
function normalizeConfiguration(source = {}) {
  ...
  if (packagedManagedPricing) {
    ...
    const autoReviewRows = packagedPricingRows().filter((row) => (
      row.provider === "openai" && row.model === "codex-auto-review"
    ));
    const hasAutoReviewRows = normalizedSourcePrices.some((row) => (
      row.provider === "openai" && row.model === "codex-auto-review"
    ));
    if (!hasAutoReviewRows) {
      for (const [index, sourceRow] of autoReviewRows.entries()) {
        const autoReview = normalizePricingRow(sourceRow, normalizedSourcePrices.length + index);
        normalizedSourcePrices.push(autoReview);
        sourceIds.add(autoReview.id);
      }
    }
  }
