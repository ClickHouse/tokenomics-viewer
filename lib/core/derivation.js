...
const CODEX_USAGE_DERIVATION_VERSION = 4;
...
function canonicalSourceContentFingerprint(fingerprint) {
  return canonicalSourceFingerprint(fingerprint)
    .split("|")
    .filter((part) => (
      !part.startsWith("analyticsDerivationVersion=")
      && !part.startsWith("codexUsageDerivationVersion=")
    ))
    .sort()
    .join("|");
}

function sameSourceContentFingerprint(left, right) {
  if (!left || !right) return false;
  const leftContent = canonicalSourceContentFingerprint(left);
  const rightContent = canonicalSourceContentFingerprint(right);
...
module.exports = {
  ...
  canonicalSourceContentFingerprint,
