...
function scheduledWeekdaysUTC(now) {
  const year = now.getUTCFullYear();
  const month = now.getUTCMonth();
  const days = [];
  for (let day = 1; ; day += 1) {
    const current = new Date(Date.UTC(year, month, day));
    if (current.getUTCMonth() !== month) break;
...
function budgetSummary(report, usageProfile, now, currentMonthName, currentMonthCostUsd, monthlyCostLimitUsd) {
  const today = dateKey(now);
  const scheduledDays = scheduledWeekdaysUTC(now);
  const todayScheduled = scheduledDays.includes(today);
  const remainingScheduledDays = scheduledDays.filter((day) => day >= today).length;
  const spendThroughYesterday = Object.entries(report.daily || {})
    .filter(([day]) => day.startsWith(`${currentMonthName}-`) && day < today)
    .reduce((total, [, stats]) => total + (stats?.costUsd || 0), 0);
  const todaySpendUsd = report.daily?.[today]?.costUsd || 0;
  const limitUsd = usageProfile.mode === "api" ? monthlyCostLimitUsd : null;
  const remainingUsd = limitUsd === null ? null : Math.max(0, limitUsd - currentMonthCostUsd);
  const overageUsd = limitUsd === null ? null : Math.max(0, currentMonthCostUsd - limitUsd);
  const todayAllowanceUsd = limitUsd !== null && todayScheduled && remainingScheduledDays > 0
    ? Math.max(0, (limitUsd - spendThroughYesterday) / remainingScheduledDays)
    : null;

  let status;
  if (usageProfile.mode === "subscription") {
    status = todayScheduled ? "subscription" : "subscription-unscheduled";
  } else if (limitUsd === null) {
    status = todayScheduled ? "no-limit" : "unscheduled";
  } else if (!todayScheduled) {
    status = "unscheduled";
  } else if (overageUsd > 0) {
    status = "over-budget";
  } else {
    status = "on-track";
  }
  ...
  return {
    ...
    limitUsd,
    spentUsd: currentMonthCostUsd,
    remainingUsd,
    overageUsd,
    usedRatio: limitUsd === null ? null : currentMonthCostUsd / limitUsd,
    baselineDailyTargetUsd: limitUsd === null ? null : limitUsd / scheduledDays.length,
    todayAllowanceUsd,
    todayRemainingUsd: todayAllowanceUsd === null ? null : Math.max(0, todayAllowanceUsd - todaySpendUsd),
    allowanceBasis: limitUsd === null
      ? "no_monthly_limit"
      : "monthly_limit_minus_spend_through_yesterday_divided_by_remaining_weekdays_utc",
    status,
  };
}
...
function webSummary(report, options = {}) {
  ...
  return {
    contractVersion: 1,
    generatedAt: report.receipt?.committedAt || now.toISOString(),
    committedAt: report.receipt?.committedAt || null,
    dataThrough: report.receipt?.dataThrough || null,
