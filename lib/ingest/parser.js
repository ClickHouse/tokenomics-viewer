...
const { diagnosticEventKey, usagePayloadHash } = require("../core/usage-event-identity");

const CODEX_PARSER_CHECKPOINT_VERSION = 3;
// Claude checkpoints did not change when Codex added payload hashes, so old
// Claude cursors remain safe. Codex v1/v2 cursors must be replayed below.
const SUPPORTED_PARSER_CHECKPOINT_VERSIONS = new Set([1, 2, CODEX_PARSER_CHECKPOINT_VERSION]);
const CODEX_USAGE_FORMAT_LEGACY = "legacy-token-count";
const CODEX_USAGE_FORMAT_RECORD = "token-usage-record";
// Checkpoints retain a bounded recent window, but a full parse keeps every
// observed response ID. Durable storage performs cross-range/source dedupe.
...
function restoreParserCheckpoint(checkpoint, sourceLabel) {
  ...
  const recentUsageResponses = checkpoint.recentUsageResponses ?? [];
  if (
    !Array.isArray(recentUsageResponses) ||
    recentUsageResponses.length > CLAUDE_REQUEST_CHECKPOINT_LIMIT ||
    recentUsageResponses.some(({ responseId, payloadHash } = {}) => (
      typeof responseId !== "string" || !responseId || !/^[a-f0-9]{64}$/.test(payloadHash)
    )) ||
    (usageFormat === CODEX_USAGE_FORMAT_RECORD && !checkpoint.recentUsageResponses)
  ) {
  ...
  return {
    ...
    usageFormat,
    recentUsageResponses: [...new Map(
      recentUsageResponses.map(({ responseId, payloadHash }) => [responseId, { responseId, payloadHash }]),
    ).values()],
...
function createLineProcessor(report, options, sourceLabel, session = null) {
  ...
  const seenCodexUsageResponses = new Map(
    codexState.recentUsageResponses.map(({ responseId, payloadHash }) => [responseId, payloadHash]),
  );
  ...
  const processor = (line, lineNo) => {
    ...
    if (json.type === "session_meta" && json.payload) {
      ...
      if (!codexState.sessionId) {
        ...
        codexState.forkedFromId = codexParentSessionId(json.payload);
    ...
    if (json.type === "token_usage_record" && json.payload) {
      ...
      const payloadHash = usagePayloadHash({
        ...
        agent: CODEX_SERVICE_AGENT,
        usage: usageFromCodexTokenUsage(json.payload.usage),
      });
      if (seenCodexUsageResponses.has(requestId)) {
        const previousHash = seenCodexUsageResponses.get(requestId);
        if (previousHash && previousHash !== payloadHash) {
          report.sources.conflictingUsageEvents += 1;
          throw new Error(`Conflicting Codex usage event ${diagnosticEventKey(`openai:${requestId}`)} in ${sourceLabel}:${lineNo}`);
        }
        report.sources.duplicateUsageEvents += 1;
        return;
      }
      seenCodexUsageResponses.set(requestId, payloadHash);
      const timestamp = new Date(json.timestamp);
  ...
  processor.checkpoint = () => {
    ...
    return {
      ...
      recentUsageResponses: [...seenCodexUsageResponses.entries()]
        .slice(-CLAUDE_REQUEST_CHECKPOINT_LIMIT)
        .map(([responseId, payloadHash]) => ({ responseId, payloadHash })),
