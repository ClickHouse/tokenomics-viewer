...
function createIngestSources({
  ...
  async function readCodexSessionHeader(source) {
    ...
    try {
      for await (const line of lines) {
        ...
        return {
          id,
          forkedFromId: codexParentSessionId(json.payload),
  ...
  async function prepareCodexForkRegistry(inputs, options) {
    ...
    const addHeader = (header) => {
      const headersBySource = headersBySession.get(header.id) || new Map();
      headersBySource.set(header.source.sourcePath, header);
      headersBySession.set(header.id, headersBySource);
    };
    for (const header of persistedHeaders) addHeader(header);
    for (const header of currentHeaders) addHeader(header);
    ...
    const collectSessionTraces = async (sessionId) => {
      ...
      const headers = [...(headersBySession.get(sessionId)?.values() || [])];
      if (headers.length === 0) return new Set();
      visiting.add(sessionId);
      const traces = new Set();
      for (const header of headers) {
        if (!header.forkedFromId) continue;
        const inheritedTraces = await collectSessionTraces(header.forkedFromId);
        for (const traceId of inheritedTraces) traces.add(traceId);
      }
      for (const header of headers) {
        const inheritedTraces = header.forkedFromId
          ? await collectSessionTraces(header.forkedFromId)
          : new Set();
        const parentHeaders = header.forkedFromId ? headersBySession.get(header.forkedFromId) : null;
        const { ownTraces, sawInheritedReplay, currentChildBoundaryTraces } = await collectCodexOwnTraceIds(
          header.source,
          inheritedTraces,
          header.forkedFromId,
          header.cwd,
          Boolean(header.forkedFromId && !parentHeaders),
        );
        if (sawInheritedReplay) replaySessions.add(sessionId);
        if (currentChildBoundaryTraces?.size > 0) {
          const boundaries = replayBoundariesBySession.get(sessionId) || new Set();
          for (const traceId of currentChildBoundaryTraces) boundaries.add(traceId);
          replayBoundariesBySession.set(sessionId, boundaries);
        }
